...
**选择vector作为容器是因为在算法实现过程中需要频繁的访问元素，而vector 在内存中以连续的方式存储元素，有助于提高缓存的局部性，从而可能提高性能。
同时在算法实现过程中需要频繁的随机访问占用某个功能部件的指令和写某个寄存器的指令，vector 提供了对元素的快速随机访问，同时访问时间复杂度为 O(1)。
同时很少涉及在序列中插入或者删除元素，所以vector是个很好的选择。**
